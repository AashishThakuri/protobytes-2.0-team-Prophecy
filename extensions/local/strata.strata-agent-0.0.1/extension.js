...
const ROLE_TOOL_ALLOWLIST = {
  architect: new Set(['readFile', 'listFiles', 'openFile', 'fetchUrl', 'manageMemory', 'getSystemStatus']),
  researcher: new Set(['readFile', 'listFiles', 'openFile', 'fetchUrl', 'manageMemory', 'createOrOverwriteFile', 'appendToFile']),
  coder: new Set(['readFile', 'listFiles', 'openFile', 'createDirectory', 'createOrOverwriteFile', 'appendToFile', 'runTerminalCommand']),
  debugger: new Set(['readFile', 'listFiles', 'openFile', 'createOrOverwriteFile', 'appendToFile', 'runTerminalCommand', 'killPort', 'killBackgroundProcesses', 'getSystemStatus']),
  data: new Set(['readFile', 'listFiles', 'openFile', 'fetchUrl', 'createOrOverwriteFile', 'appendToFile', 'manageMemory']),
  devops: new Set(['readFile', 'listFiles', 'openFile', 'createOrOverwriteFile', 'appendToFile', 'runTerminalCommand', 'killPort', 'killBackgroundProcesses', 'getSystemStatus', 'createBackup']),
};
...
- Treat "Nanobanana Pro" as the default image/visual generation engine available behind the Gemini API and environment.
- You do NOT directly output binary images; instead you:
  - Design high-quality, implementation-ready prompts for Nanobanana Pro (style, composition, lighting, camera, color, typography, etc.).
  - When tools are wired for image generation, use them via the existing tool interfaces (for example, fetchUrl or runTerminalCommand calling Nanobanana-related APIs); do NOT invent new tool types.
- Generated designs must be high-quality, context-aware, and aligned with the current Strata project.
...
<head>
  <meta charset="UTF-8" />
  <meta http-equiv="Content-Security-Policy" content="default-src 'none'; frame-src 'self' * https: data: blob:; img-src 'self' 'unsafe-inline' https: data: vscode-resource: vscode-webview: blob:; media-src 'self' * https: data: blob:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' 'unsafe-eval' blob: https:; script-src-elem 'self' 'unsafe-inline' 'unsafe-eval' blob: https:; worker-src 'self' blob:; connect-src 'self' * https: wss: blob:;" />
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function saveToolLog() {
      try {
        localStorage.setItem(TOOL_LOG_KEY, JSON.stringify(toolLog));
      } catch (e) {
        // ignore
      }
    }
    ...
    function loadToolLog() {
      let saved = null;
      try {
        saved = localStorage.getItem(TOOL_LOG_KEY);
      } catch (e) {
        saved = null;
      }
      if (!saved) {
